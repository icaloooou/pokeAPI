...
# função deve rejeitar valores não-numéricos ou numerais não-inteiros nesse parâmetro.
...
# em minúsculas, MAIÚSCULAS ou até mesmo MiStUrAdO. Lembre-se dos métodos
# lower() e upper() da classe string.
...
def numero_do_pokemon(nome):
    check_str(nome)
    nome_minusculo = nome.lower()  # padroniza as strings q recebe em minusculo
...
def tipos_do_pokemon(nome):
    nome_minusculo = nome.lower()
    lista_tipos = []
    url = f"http://pokeapi.co/api/v2/pokemon/{nome_minusculo}/"
    retorno = requests.get(url)
    if retorno.status_code == 404:
        raise PokemonNaoExisteException
    dici = retorno.json()
    for tipos in dici['types']:
        nome_tipo = tipos['type']['name']
        lista_tipos.append(dic_tipos[nome_tipo])
    return lista_tipos
...
def evolucao_anterior(nome):
    ...
    if retorno.status_code == 404:
        raise PokemonNaoExisteException
    dici = retorno.json()
    if dici['evolves_from_species'] is None:
        return None
    else:
...
def nivel_do_pokemon(nome, experiencia):
    url = f"http://pokeapi.co/api/v2/pokemon-species/{nome.lower()}/"
    retorno = requests.get(url)
    if retorno.status_code == 404:
        raise PokemonNaoExisteException
    dic = retorno.json()
    id = int(dic['growth_rate']['url'][-2])
    url_growth_rate = f"https://pokeapi.co/api/v2/growth-rate/{id}/"
    retorno2 = requests.get(url_growth_rate)
    dicXp = retorno2.json()
    if experiencia == 0:
        return 1
    for i in range(1, 100):
        if experiencia <= dicXp['levels'][i]['experience']:
            return i
